...
class Maze {
	...
	public ArrayList<Square> storePath() {
		ArrayList<Square> storedPathArrayList = new ArrayList<>();
		
		if (finish.getPrevious() == null) {
			return storedPathArrayList;
		}
		
		Square current = finish;
		storedPathArrayList.add(finish);
		
		while (current != start) {
			storedPathArrayList.add(current.getPrevious());
			current = current.getPrevious();
		}
		storedPathArrayList.add(start);
		//Collections.reverse(storedPathArrayList);
